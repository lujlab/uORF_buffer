...
for i in [k / 10 for k in range(0, 11)]:
    for j in [k / 10 for k in range(0, 11)]:
        for k in r_in_lines:
            ...
            tmp['P_clear_up'] = 1
            tmp['P_clear_down'] = 1  # set as double dissociation mode
...
Sim_multi_condition(condition_list, 'sim_result/1000Lambda_runif__11P_s2p_u__11P_s2p_m__v8_double_dissociation.txt')
